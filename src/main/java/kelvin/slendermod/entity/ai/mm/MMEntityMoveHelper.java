package kelvin.slendermod.entity.ai.mm;
...
public class MMEntityMoveHelper extends MoveControl {
    ...
    public MMEntityMoveHelper(MobEntity entity, float maxRotate) {
        super(entity);
    ...
    public void tick() {
        if (this.state == MoveControl.State.STRAFE) {
            ...
            if (pathnavigate != null) {
                ...
                if (nodeprocessor != null && nodeprocessor.getDefaultNodeType(this.entity.world, MathHelper.floor(this.entity.getX() + (double) f7), MathHelper.floor(this.entity.getY()), MathHelper.floor(this.entity.getZ() + (double) f8)) != PathNodeType.WALKABLE) {
